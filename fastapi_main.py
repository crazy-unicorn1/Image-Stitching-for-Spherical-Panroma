...
from stitch import convert_to_array, decode_base64_image, read_images_fastapi_from_json
...
async def stitch_images(request):
    ...
    try:
        ...
        input_images = read_images_fastapi_from_json(json_data)

        # Handle the case when images are not properly sent
        if len(input_images) != len(json_data):
            raise HTTPException(status_code=500, detail={'error_message': f'Failed to read images. Expected {str(len(json_data))} , got {str(len(input_images))} images encoded as string.', 'error_code': 501})
        ...
        if len(input_images) == 0:
            ...
            raise HTTPException(status_code=500, detail={'error_message': 'Failed to read images. Expected 2 or more images encoded as string.', 'error_code': 501})
        ...
        for idx, x in enumerate(kpts):
            if len(x) < 10:
                raise HTTPException(status_code=500, detail={'error_message': f'Image {idx+1} has less than 10 keypoints.Please retake the photos.', 'error_code': 502})
        ...
        raise HTTPException(status_code=500, detail={'error_message': "Failed to stitch images.", 'error_code': 500})
...
class ImageData(BaseModel):
    image: str
    motionData: Optional[str]
    orientationData: Optional[str]
...
@app.post("/")
async def request_stitching(data: List[ImageData]):
    results, status_code, headers = await stitch_images(data)
    return results
    
@app.get("/")
async def request_stitching():
